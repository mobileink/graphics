...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  \pgfmathsetmacro\x{easeInQuadratic(\iFrame,\rFromXa,\rDeltaXa,\value{iFrames})};
...
%%%%%%%%%%%%%%%%
 ...
 \draw[very thin,color=gray] (-1,0) grid (3.2,1.2);
  ...
  %% some ref lines
  %% \draw[gray] (1/{sqrt(2)},0) node[color=black, below] {$\frac{1}{\sqrt{2}}$}
  %% -- (1/{sqrt(2)},1/{sqrt(2)});
  %% \draw[gray] (0,1/{sqrt(2)}) -- (1/{sqrt(2)},1/{sqrt(2)});
...
%  \fill[black] (1,e) node[anchor=south west] {$(1,e)$} circle (.7pt);
 ...
 %% dot (e,0)
  ...
  %% %% dot (e,1/e)
  %% \fill[red] (e, 1/e) node[right] {$C'$} circle (.4pt);
