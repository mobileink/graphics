...
``Topology (from the Greek τόπος, "place", and λόγος, "study") is the
mathematical study of shapes and spaces.  It is an area of mathematics
concerned with the properties of space that are preserved under
continuous deformations including stretching and bending, but not
tearing or gluing. This includes such properties as connectedness,
continuity and boundary.'' \url{http://en.wikipedia.org/wiki/Topology}

That sounds like a pretty good description of what a graphics language
has to deal with.
...
term \([M[x:=N]\).''\marginnote{\citet{lalement_computation_1993}}
  ...
  \begin{remark}
    In Z's set theoretic world, a function just \emph{is} a set of
    ordered pairs.  Lambda's view on functions comes a little closer
    to the intuitive notion of function as office or service.  It
    doesn't matter what a function \emph{is}; what matters is whether
    we can compute it or not.  So a function in lambda-world is a kind
  ...
  \begin{remark}
    RVs?  Huh?  We will need these when it comes time to talk about
    data/content.  If we want to support data visualization
    (data-driven graphics, etc.) then we will have to deal with sets
    of data points--graphs--whose function is unknown, or which may
    not be members of the graph of any function.  Regression etc. -
    critical role of concept of error, curve fitting etc.  The best
    way to deal with this is in terms of random variables (with
    unknown probability distributions).  A complete account of this
    approach to dealing with data is beyond the scope of this paper,
    but we should at least sketch the idea.
  \end{remark}
  ...
  and is the most general notion of a mathematical "space" that allows
  for the definition of concepts such as continuity, connectedness,
  and convergence. Other spaces, such as manifolds and metric spaces,
  are specializations of topological spaces with extra structures or
  constraints. Being so general, topological spaces are a central
  unifying notion and appear in virtually every branch of modern
  mathematics.'' \url{http://en.wikipedia.org/wiki/Topological\_spaces}
  ...
  \begin{remark}
    Transforms as
    \href{http://en.wikipedia.org/wiki/Embedding}{embeddings} of
    one \cspace{} in another?  \(\Omega_1\hookrightarrow\Omega_2\)
  \end{remark}
  ...
  to image, we have to \emph{embed} the graph in a definite \cspace{}.
  When we do this, we end up with a \textit{curve}, which is the
  ...
  \begin{remark}
    ...
    Doesn't work for random data.  So if we want to support data
    visualization we will need something like a random variable.
  \end{remark}
  ...
  embedding is given by some injective and structure-preserving map f
  : X → Y. The precise meaning of "structure-preserving" depends on
  the kind of mathematical structure of which X and Y are
  instances. In the terminology of category theory, a
  structure-preserving map is called a morphism.  The fact that a map
  f : X → Y is an embedding is often indicated by the use of a "hooked
  arrow", thus: \(f : X \hookrightarrow Y\). On the other hand, this
  notation is sometimes reserved for inclusion maps.''
  \url{http://en.wikipedia.org/wiki/Embedding}

  \begin{remark}
    Embedding is a key concept; we treat ``drawing'' curves in
    a \cspace{} in terms of an embedding of a function in a \cspace{}.
    Or something like that, anyway.
  \end{remark}


  \begin{remark}
    The wikipedia article talks in terms of topologies.  Is the graph
    of a function a topology?  Is it correct to view the curve of a
    function in a \cspace{} as an embedding of one topological space
    in another?  ``In general topology, an embedding is a
    homeomorphism onto its image''
    \url{http://en.wikipedia.org/wiki/Embedding} ``Roughly speaking, a
    topological space is a geometric object, and the homeomorphism is
    a continuous stretching and bending of the object into a new
    shape. Thus, a square and a circle are homeomorphic to each other,
    but a sphere and a donut are not.''
    \url{http://en.wikipedia.org/wiki/Homeomorphism}
  \end{remark}
  ...
  \begin{remark}
    Curve of a function = graph of the function \emph{embedded}
    (=drawn) in \cspace{}.
  \end{remark}

  \begin{remark}
    Functions with infinite domains obviously cannot be ``drawn'' on a
    finite physical surface.  So when we say ``curve'' what we really
    mean is a fragment of a curve.
  \end{remark}
  ...
  \begin{remark}
    Figure as \href{http://en.wikipedia.org/wiki/Embedding}{embedding}?
  \end{remark}
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  \section{Animation}
  \label{sect:animation}

  Animation requires:

  \begin{itemize}
  \item start and end points
  \item distance interval (determined by domain of start/end pts)
  \item interpolation curve - path along which drawing occurs
  \item acceleration curve - governs placement of interpolated points
  \item sample rate - in units of distance rather than time, e.g. so many samples per unit distance
  \item sample range and index - sample rate of 100 determines range of [1..100]
  \end{itemize}

  The acceleration curve (function) together with the sample index
  determines the selection of points in the distance interval for
  mapping by the interpolation curve.  In other words, the sample rate
  tells us how many points to sample from the x interval, and the
  acceleration function tells us which ones (how they are
  distributed).  (The x interval is normalized to 1).
    ...
    %%%%%%%%%%%%%%%%
        ...
        \noindent
            ...
            \noindent
                ...
                %%%%%%%%
                \subsection{Postscript Character Space}
                \label{subs:pschar}

                \hyperref{http://partners.adobe.com/public/developer/en/font/T1\_SPEC.PDF}{Adobe Type 1 Font Format}, p. 25

                ``In the PostScript language, characters have their
                own coordinate system distinct from the coordinate
                system used by a speciﬁc device. The coordinate system
                in which characters are deﬁned is called character
                space, the coordinate system used by a device is
                called device space, and the coordinate system used in
                PostScript language programs for placing objects on a
                page is called user space.'' (p. 25)

                ``Type 1 font programs generally use a 1000 to 1
                scaling matrix for the deﬁnition of the relationship
                of character space units to user space units. The
                FontMatrix value in these fonts is typically [0.001 0
                  0 0.001 0 0]. Thus, 1000 character space units will
                scale down to 1 user space unit (before application of
                the makefont or scalefont operators in a PostScript
                language program). This allows character space
                coordinates to be expressed in integer values without
                signiﬁcant loss of precision for most font designs.
                If additional precision is necessary, expressions such
                as 145 10div may be used to provide a number (in this
                case, 14.5) that cannot be expressed directly in the
                charstring format.''

                ``Character Paths A character is made up of PostScript
                language code that draws the character in character
                space. The first step in preparing the con- tents of a
                charstring is to develop a PostScript language program
                that defines the character outline in character
                space. An outline is defined by building a path with
                the moveto, lineto, curveto, closepath, rlineto,
                etc. operators.'' p. 26
