...
%%%%%%%%
\subsection{Requirements for a Vector Graphics Language}
\label{subs:reqts}
...
%%%%%%%%
\section{The Z Formal Specification Notation}
\label{subs:z}

The Z notation is clear, concise, precise, etc. and \textit{very}
expressive.  It allows us to express formal definitions with a minimum
form and maximal information.\marginnote{TDOD: why Z is so great}
...
definitions.  We used the language defined by the Z\marginnote{Z is
  pronounced ``Zed''.} specification
notation.\sidenote{\citetalias{z-iso-13568}} Since the definition of Z
...
Any function whose range is \RR{} may serve as a positioning functon,
and may occur in a path/figure expression wherever a point value is
expected.
...
Type: \(\mathds{R}\rightarrow\mathds{R}\)
...
\begin{remark}
  Save the results of immediate evaluation of expressions to the cs.
\end{remark}

%%%%%%%%
\subsection{Just-in-Time Evaluation}
\label{subs:lazy}

a/k/a Lazy Evaluation

\begin{remark}
  Save the expressions to the cs, rather than their evaluations.
\end{remark}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\part{Structure of Coordinate Spaces}
\label{sect:cspaces}
...
\item [angle of incidence] (azimuth?) relative to x axis in absolute space.
...
, \((x,y)\)
orientation: every point in the first quadrant is
(+,+)\sidenote{(+,+) \(\rightarrow\) positive in both coordinates}
with first coordinates increasing to the right and second coordinates
increasing upwards, and in positioning a point, the $x$-coordinate
``comes first''\sidenote{\''Comes first'': the first
  element of a coordinate pair \((a,b)\) maps to the \(x\) axis, the
...
specification (see \ref{sect:axes}).  The order of axes, by contrast,
is a property of the \cspace{} that contains (or uses) them; an axis
has no way of knowing (so to speak) how it will be used by a \cspace;
it doesn't even ``know'' how many other axes might be involved.  So it
